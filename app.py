...
if app_mode == "📈 Backtest Lịch sử 3 năm":
    ...
    # Lưu trữ tín hiệu tách biệt theo đúng ngày hiện tại
    ...
    for item in st.session_state.portfolio_active:
        ...
        try:
            hist_df = yf.download(item['Mã CP'] + ".VN", period="1mo", progress=False)
    ...
    # Hiển thị Dashboard chỉ lấy đúng dữ liệu của ngày hiện tại (today_str)
    display_signals_df = st.session_state.daily_signals_store.get(today_str, pd.DataFrame())

    st.subheader(f"1️⃣ Danh sách Cổ phiếu Phát Tín Hiệu Mua (Ngày {today_str})")
    if not display_signals_df.empty:
        ...
        st.info("⏳ Chưa có mã nào kích hoạt điểm mua mới trong ngày hôm nay.")
